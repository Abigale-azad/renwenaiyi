...
export function resolveBinding(
    ...
    const removeLegacyGrowth = () => {
        const ids = new Set(loadWorldBooks().filter(book => book.description?.startsWith("auto-personality-growth:")).map(book => book.id));
        resolved.worldBookIds = (resolved.worldBookIds || []).filter(id => !ids.has(id));
    };
    if (!characterId) { removeLegacyGrowth(); return resolved; }
    ...
    // Legacy growth books are recovery sources only. Approved growth is injected
    // separately by character ID and cannot inherit global worldbook bindings.
    removeLegacyGrowth();
