...
export function resolveBinding(
    ...
    // Character-specific personality growth books are generated by the memory
    // pipeline and always accompany that character without replacing manual bindings.
    const growthMarker = `auto-personality-growth:${characterId}`;
    const growthIds = loadWorldBooks()
        .filter(book => book.description === growthMarker)
        .map(book => book.id);
    if (growthIds.length > 0) {
        resolved.worldBookIds = [...new Set([...(resolved.worldBookIds || []), ...growthIds])];
    }
