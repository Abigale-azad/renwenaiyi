...
const MUSIC_SWITCH_PARAMETER_SCHEMA = JSON.stringify({
    type: "object",
    properties: {
        action: { type: "string", description: "next|prev|pause|resume|stop|shuffle|sequence|repeat-one" },
    },
    required: ["action"],
});
...
const FLOWUS_USAGE_GUIDE = [
    ...
    "",
    "【优先规则·重要】",
    "- 当{{user}}说“记一下/帮我记着/存个待办/加到待办/别忘了提醒我/这件事记下来”等明确要备忘时，调用「创建待办」。标题用一句话概括，note 里放细节和来源。",
    "- 当{{user}}说“我今天有什么事/待办列表/还有什么没做/查一下待办/任务清单”等问待办时，调用「查询待办」，不传 status 就返回全部。",
    "- 当{{user}}说“这件事做完了/完成待办/标记完成/勾掉”等明确完成某件事时，先调「查询待办」找到对应记录，再调「完成待办」。",
    "- 当{{user}}说“存一下/收藏一下/保存这段话/记到资料里/存到收件箱/这段很重要”等明确要保存聊天内容时，调用「保存聊天资料」。title 取前 60 字，tags 按内容分类，比如[\"想法\"][\"灵感\"][\"引用\"][\"重要\"]。",
    "- 当{{user}}说“查一下知识库/搜一下资料/找一下之前记的/FlowUs 里有没有”等要检索时，调用「搜索资料」。",
    "- 当{{user}}说“查一下多维表/看看那个表/看板里有什么”等明确要看某个多维表时，调用「查询多维表」；需要知道 databaseId 时先调「搜索资料」找。",
    "- 不要主动用 FlowUs 做任何操作，只有{{user}}明确提到要记、要存、要查的时候才调用。日常聊天里别没事就查待办或存资料。",
    "- 创建/保存类动作只输出执行动作指令，不要附加解释；查询类动作拿到结果后用自然语言回复{{user}}。",
    "",
    "动作：创建待办",
