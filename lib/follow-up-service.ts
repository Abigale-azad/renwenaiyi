...
async function prepareMusicCompanion(request: MusicCompanionRequestDetail): Promise<void> {
    ...
    try {
        ...
        await saveRoleSongMemories(request.characterId, selected.map(({ item, track }) => ({ trackId: String(track.id), understanding: item.understanding?.trim(), selectedCount: 1, lastSelectedAt: now }))); bridge.setPlayMode("sequence"); await bridge.addToQueue(queue, { replace: true, playFirst: true }); updateMusicCompanion({ status: "ready", selectedTrackIds: queue.map(track => track.id), plans });
        const playlistTitle = parsedSecond?.playlistTitle?.trim().slice(0, 32) || "这次想和你听";
        const playlistThought = parsedSecond?.playlistThought?.trim().slice(0, 180) || "顺着今晚的情绪，替你把这些歌排在了一起。";
        pushChatMessage({ sessionId: request.sessionId, role: "assistant", content: `${playlistTitle}｜${queue.length}首陪听歌单\n${playlistThought}`, mediaType: "music_companion_card", mediaData: { musicCompanionTitle: playlistTitle, musicCompanionThought: playlistThought, musicCompanionTracks: selected.map(({ item, track }) => ({ trackId: `netease_${track.id}`, title: track.title, artist: track.artist, coverUrl: track.coverUrl, understanding: item.understanding?.trim().slice(0, 180) })) } });
        dispatchMusicProgress(request, 100, `已选好 ${queue.length} 首，开始播放`, "ready");
