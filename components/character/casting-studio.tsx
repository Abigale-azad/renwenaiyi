...
export function CastingStudio({ worldGroups, initialWorldId, onClose, onSaved, onNotice }: { worldGroups: CharacterWorldGroup[]; initialWorldId: string; onClose: () => void; onSaved: () => void; onNotice: (text: string) => void }) {
  ...
  return createPortal(<div className="casting-studio fixed inset-0 z-[9000000] overflow-y-auto" role="dialog" aria-modal="true" aria-label="妃卡" onPointerDown={(event) => event.stopPropagation()} onClick={(event) => event.stopPropagation()}>
    <header className="casting-studio-header sticky top-0 z-20 flex items-center justify-between px-4 py-4">
      <button
    ...
    <main className="casting-studio-body p-4 pb-14 max-w-xl mx-auto">
